...
def get_prostate_mask(data):
    return data == 1


def get_bladder_mask(data):
    return data == 2


def get_rectum_mask(data):
    return data == 3
...
def evaluate(args):
    ...
    masking_functions = (get_prostate_mask,
                         get_bladder_mask,
                         get_rectum_mask)
...
def plot_prediction(df):
    ...
    for index, score in enumerate(df.columns):
        values = df.values.T[index]
        scores[score] = values[(np.isnan(values) == False) & (values!=0)]
    ...
    plt.savefig("validation_scores_boxplot_nonzero.png")
    plt.close()
