...
from engine.generator import get_training_and_validation_and_testing_generators25d
...
def train(args):
    ...
    if not args.overwrite and os.path.exists(config["model_file"]):
        ...
        if args.model == "unet":
            ...
            print("init unet_v2 model")
            model = unet_model_25d_net2(input_shape=config["input_shape"],
                                        n_labels=config["n_labels"],
                                        initial_learning_rate=config["initial_learning_rate"],
                                        deconvolution=config["deconvolution"],
                                        batch_normalization=True,
            ...
            model = segnet25d(input_shape=config["input_shape"],
                              ...
                              loss_function=args.loss,
                              labels=config["labels"])
        elif args.model == "segnet_v2":
            print("init segnet_v2 model")
            model = segnet25d_net2(input_shape=config["input_shape"],
                                   n_labels=config["n_labels"],
                                   initial_learning_rate=config["initial_learning_rate"],
                                   depth=args.depth_unet,
                                   n_base_filters=args.n_base_filters_unet,
